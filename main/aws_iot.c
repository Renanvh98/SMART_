...
void aws_iot_task(void *param) {
    ...
    while((NETWORK_ATTEMPTING_RECONNECT == rc || NETWORK_RECONNECTED == rc || SUCCESS == rc)) {
        ...
        sprintf(cPayload, "%s : %.2f, %s : %.2f, %s : %.2f, %s : %.2f, %s : %.2f", "voltage", getVoltage(), "current", getCurrent(), "power", getPower(), "total", getTotalEnergy(), "custo", getCustoPorHora());
        //getVoltage(), getCurrent(), getPower(), getTotalEnergy(), getCustoPorHora()
       // voltage\":\"%.1f\",\"current\":\"%.2f\",\"power\":\"%.2f\",\"total\":\"%.1f\",\"custo\":\"%.2f
